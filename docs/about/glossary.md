...
### Coordinate OTUs
A set of [OTUs](#otu-operational-taxonomic-unit) that are conceptually the same thing, as inferred by their nomenclatural classification. For example, imagine the same species described with different names multiple times in history. The data circumscribing each individual description _may_ be used to define a single OTU, perhaps the description in 1910 is one OTU, then 1940 another, then 1960 yet another. `Coordinate` related functions bring all the biological data linked to these OTUs into one place. Roughly, biological synonymy. 
...
The biological unit that is useful to work with. All biological data are linked to an OTU in TaxonWorks, not directly to a taxon name.  This abstraction is necessary because 1)  different species may have an identical taxon name; 2) not all species are described yet, and have taxon names; 3) not all units of work coordinate 1:1 with a taxon (e.g. a water sample containing unknown species, a drawer of specimens, a taxon concept in progress); 4) 
