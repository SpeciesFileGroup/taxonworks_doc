...
* `Rows` ara `OTUs`, `Collection Objects` and other objects in the database which could be added to a matrix. The same OTU or any other object could be added to as many matrices as needed.
...
- To add a new `Descriptor`, click the `New Descriptor` link at the top off the interface. Select the descriptor `Type`. The available types are:
  ...
  - `Gene` - ???
  - `Free text`
  - `Meadia` - a specialized descriptor designed for attachment of `Depictions`, which will serve as a visual observation in the image matrix
