...
## FAQ related to how to add alternative descriptor names for interactive key
- The procedure is the same as for automatically generated description, the alternative values could be provided in the field `Key name`

## FAQ related to how organize descriptors in the interactive key.
- By default, the descriptors are organized by their separating power, so that the descriptors which break the group more evenly positioned closer to the top.
- That order could be altered by adding a weight to each descriptor, using `Show more` option in the Edit descriptor interface. The descriptors with higher weight will be positioned closer to the top of the key.
